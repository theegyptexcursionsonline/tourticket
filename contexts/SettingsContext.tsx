...
export function SettingsProvider({
  ...
  const localeLanguage =
    initialLocale && languages.find((lang) => lang.code === initialLocale)
      ? (languages.find((lang) => lang.code === initialLocale) as Language)
      : fallbackLanguage;
  ...
  useEffect(() => {
    ...
    const matched = languages.find((lang) => lang.code === initialLocale);
    if (!matched) return;
    if (matched.code !== selectedLanguage.code) {
      setSelectedLanguage(matched);
    }
  }, [initialLocale, selectedLanguage.code, setSelectedLanguage]);
  ...
  const t = useCallback((key: string, params?: Record<string, string | number>): string => {
    const localeMessages = messageCatalog[selectedLanguage.code] || messageCatalog.en;
    const translation =
      getMessageByKey(localeMessages, key) ||
      getMessageByKey(messageCatalog.en, key) ||
      key;
    
    if (params) {
      return Object.entries(params).reduce((acc, [paramKey, paramValue]) => {
        const value = String(paramValue);
        return acc.replaceAll(`{{${paramKey}}}`, value).replaceAll(`{${paramKey}}`, value);
      }, translation);
    }
