...
describe('EEO options are matched by their immutable pricingKey', () => {
  // EEO booking options persist neither `id` nor `_id`; checkout identifies
  // them by pricingKey, so the authoritative helper must resolve on it.
  const tour = {
    discountPercent: 10,
    discountPrice: 60,
    bookingOptions: [
      { pricingKey: 'private-abc', price: 100, applyTourDiscount: true, timeSlots: [{ time: '10:00', price: 150 }] },
      { pricingKey: 'group-def', price: 40 },
    ],
  };

  it('resolves the option by pricingKey and applies its discount', () => {
    expect(authoritativeBasePrice(tour, {
      selectedBookingOption: { pricingKey: 'private-abc', price: 1 },
    })).toBe(90);
  });

  it('combines pricingKey resolution with a slot override', () => {
    expect(authoritativeBasePrice(tour, {
      selectedBookingOption: { pricingKey: 'private-abc' },
      selectedTime: '10:00',
    })).toBe(135);
  });

  it('leaves a non-opted option at full price under pricingKey resolution', () => {
    expect(authoritativeBasePrice(tour, {
      selectedBookingOption: { pricingKey: 'group-def', price: 2 },
    })).toBe(40);
  });
});
