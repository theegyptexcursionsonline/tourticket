...
describe('per-person add-on quantity bounds', () => {
  ...
  it('re-clamps selected per-person add-ons when the paying party shrinks', () => {
    expect(clampSelectedPerPersonAddOns(
      { meal: 4, photo: 3, disabled: 0 },
      [
        { id: 'meal', perGuest: true },
        { id: 'photo', perGuest: false },
        { id: 'disabled', perGuest: true },
      ],
      1,
      1,
    )).toEqual({ meal: 2, photo: 3, disabled: 0 });
  });
});
