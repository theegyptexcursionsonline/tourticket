...
const SearchClient: React.FC<SearchClientProps> = ({ initialTours = [], categories = [], destinations = [] }) => {
  ...
  const [isLoading, setIsLoading] = useState(false); // start false to avoid immediate loader flicker
  ...
  // avoid router.replace on initial load (we'll sync from URL but not push)
  const isFirstMount = useRef(true);

  // parse initial filters from URL on mount (read-only, won't replace on mount)
  useEffect(() => {
    ...
    setSortBy(sort);
    // mark first mount done after initial sync
  ...
  const clearAllFilters = useCallback(() => {
    ...
    setSelectedRatings([]);
    setSortBy('relevance');
    // reflect in URL (replace so user doesn't get extra history entries)
    router.replace(pathname, { scroll: false });
  }, [pathname, router]);

  // Fetch tours whenever filters (debouncedQuery etc.) change.
  ...
  useEffect(() => {
    ...
    const fetchTours = async () => {
      ...
      // update URL only after first mount (so we don't replace on initial param-sync)
      const newQuery = params.toString();
      const newUrl = newQuery ? `${pathname}?${newQuery}` : pathname;
      if (!isFirstMount.current) {
        // replace so user doesn't get many history entries
        router.replace(newUrl, { scroll: false });
      }

      try {
        const res = await fetch(`/api/search/tours?${params.toString()}`, { signal });
        if (!res.ok) throw new Error('Failed to fetch tours');
        const data = await res.json();
        if (!didCancel) {
          // Expecting backend to return an array of tours
  ...
  // Sidebar component (declared inline to access parent state)
  const FilterSidebar = () => (
    <aside className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 sticky top-6">
      ...
      <div className="space-y-6">
        ...
        {/* Price Range (single max slider intentionally kept) */}
        <div>
