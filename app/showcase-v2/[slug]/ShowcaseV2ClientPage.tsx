...
export default function ShowcaseV2ClientPage({ tour, reviews, widgetConfig }: ShowcaseV2ClientPageProps) {
  ...
  return (
    <div className="pt-20 min-h-screen bg-gray-50">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
          <div className="max-w-5xl mx-auto">
            <nav className="flex items-center gap-2 text-white/80 text-sm mb-4">
              ...
              <span className="text-indigo-300">{tour.title}</span>
            </nav>
                  ...
                  <ul className="grid md:grid-cols-2 gap-4">
                    {tour.highlights.map((highlight, index) => (
                      <li key={index} className="flex items-start gap-3">
                        <span className="flex-shrink-0 w-6 h-6 bg-indigo-100 rounded-full flex items-center justify-center mt-0.5">
                    ...
                    <div key={index} className="flex gap-4">
                      <div className="flex-shrink-0">
                        ...
                        {index < tour.itinerary!.length - 1 && (
                          <div className="w-px h-full bg-indigo-200 mx-auto mt-2" />
                        )}
                      </div>
      ...
      {/* Foxes Booking Floating Widget - ONLY script needed */}
      <Script
        src={`${widgetConfig.apiUrl}/widget/foxes-booking-v2.js`}
        strategy="lazyOnload"
