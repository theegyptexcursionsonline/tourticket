...
export async function GET(request: NextRequest, context: { params: Promise<{ executionId: string }> }) {
  ...
  const matches = receipt.appliedVersion === effective.version && JSON.stringify(receipt.effectivePrices) === JSON.stringify(effective.prices) && receipt.currency === effective.currency;
