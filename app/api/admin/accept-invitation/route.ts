...
async function POSTHandler(request: NextRequest) {
  try {
    ...
    registerAdminAuditActor({
      ...
      tenantIds: user.tenantIds || ['default'],
    ...
    if (granted.tenantIds) {
      set.tenantIds = granted.tenantIds;
    }
    if (requiresPasswordSetup) {
      set.password = await bcrypt.hash(password as string, 10);
    }

    // The token is consumed in the same conditional write that grants access.
    // A double click or concurrent request can therefore succeed only once.
