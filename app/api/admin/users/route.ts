...
export async function GET(request: NextRequest) {
  ...
  await dbConnect();
  try {
    const users = await User.find({}).select('name firstName lastName email createdAt authProvider phone firebaseUid isActive deactivatedAt').sort({ createdAt: -1 }).lean();
