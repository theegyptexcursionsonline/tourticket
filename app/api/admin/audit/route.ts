...
export async function GET(request: NextRequest) {
  ...
  const scopedFilters = { ...filters, tenantIds: 'default' };
  ...
  if (searchParams.get('format') === 'csv') {
    ...
    return new NextResponse(stream, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="eeo-admin-audit-${new Date().toISOString().slice(0, 10)}.csv"`,
  ...
  const [rows, total, actors, resourceTypes, actions, outcomes] = await Promise.all([
    ...
    AdminMutationAudit.distinct('resourceType', { tenantIds: 'default' }),
    AdminMutationAudit.distinct('action', { tenantIds: 'default' }),
    AdminMutationAudit.distinct('outcome', { tenantIds: 'default' }),
  ]);
