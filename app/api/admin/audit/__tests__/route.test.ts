...
describe('GET /api/admin/audit', () => {
  ...
  it('keeps the main portal query scoped to main EEO and returns scoped stats', async () => {
    const response = await GET(makeRequest('?action=update&limit=25'));
  ...
  it('logs a successful export and includes the detailed evidence columns', async () => {
    ...
    const response = await GET(makeRequest('?format=csv')) as unknown as { status: number; _body: ReadableStream<Uint8Array> };
