...
export async function POST(request: NextRequest) {
  ...
  if (
    !['setup', 'enable', 'resume-recovery', 'acknowledge', 'regenerate', 'reset'].includes(action)
    || code.length > 64
  ) {
  ...
  const user = await User.findById(auth.userId)
    .select(
      ...
      + '+twoFactorLastUsedStep email firstName lastName role permissions '
  ...
  if (action === 'setup') {
    ...
    const uri = buildAuthenticatorUri(user.email, secret);
  ...
  if (action === 'acknowledge') {
    if (!user.twoFactorEnabled || !user.twoFactorRecoveryPending) {
      return NextResponse.json(
        { success: false, error: 'Recovery-code confirmation is not pending.' },
        { status: 409 },
      );
    }
  ...
  const verified = await verifyAndConsumeUserSecondFactor(user, code);
  if (!verified) return invalidCodeResponse();
