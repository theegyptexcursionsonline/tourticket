 ...
 * form's draft, but only as translatable content: never as scope or identity.
 */
...
describe('POST /api/admin/translate/stream', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    sseEvents = [];
    verifyAdmin.mockResolvedValue({ id: 'admin-1', role: 'admin' });
    ...
    mockAttractionPageFindById.mockResolvedValue(null);
  ...
  it('rejects an unauthenticated request before touching the database', async () => {
    verifyAdmin.mockResolvedValue(NextResponse.json({ error: 'Unauthorized' }, { status: 403 }));
  ...
  it('persists one locale at a time under the tenant filter', async () => {
    await post({ modelType: 'destination', id: 'dest-1', sourceDraft: DRAFT_WITH_CAPTIONS });
    ...
    for (const [filter, update] of mockDestinationUpdate.mock.calls) {
      expect(filter).toEqual({ _id: 'dest-1', ...DEFAULT_TENANT_FILTER });
  ...
  it('ignores draft attempts to redirect scope, identity, or stored translations', async () => {
    await post({
    ...
    expect(mockDestinationFindOne).toHaveBeenCalledWith({ _id: 'dest-1', ...DEFAULT_TENANT_FILTER });
    for (const [filter] of mockDestinationUpdate.mock.calls) {
      expect(filter).toEqual({ _id: 'dest-1', ...DEFAULT_TENANT_FILTER });
    }
    for (const [, update] of mockDestinationUpdate.mock.calls) {
  ...
  it('carries an attraction-page draft through to the translator', async () => {
    mockAttractionPageFindById.mockResolvedValue({
      _id: 'page-1',
      title: 'Karnak Temple',
