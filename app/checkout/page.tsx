...
const SummaryItem: React.FC<{item: CartItem}> = ({ item }) => {
  ...
  return (
    <div className="flex gap-4 py-4">
      <Image src={item.image || ''} alt={item.title} width={64} height={64} className="w-16 h-16 object-cover flex-shrink-0" />
    ...
    <aside className="bg-white border-t border-b border-l border-slate-200 p-6 shadow-xl sticky top-28">
      ...
      <div className="mt-4">
        <div className="flex gap-2">
          <input type="text" placeholder="Promotional code" value={promoCode} onChange={(e) => setPromoCode(e.target.value)} className="flex-1 px-4 py-2 border border-slate-300 placeholder:text-slate-400 focus-visible:ring-2 focus-visible:ring-emerald-200 transition" />
          <button onClick={applyPromoCode} type="button" className="px-4 py-2 bg-slate-900 text-white font-semibold hover:bg-slate-800 transition">Apply</button>
        </div>
      </div>
      <div className="mt-6 hidden lg:block">
        <button type="submit" form="checkout-form" disabled={isProcessing} className="w-full py-4 bg-red-600 text-white font-extrabold text-lg hover:bg-red-700 active:translate-y-[1px] transform-gpu shadow-md transition disabled:bg-red-400 disabled:cursor-not-allowed flex items-center justify-center">
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <button
            ...
            className={`flex flex-col items-center gap-2 p-4 border border-slate-200 transition-shadow ${
              paymentMethod === 'card'
        ...
        <AnimatePresence mode="wait">
          <motion.div key={paymentMethod} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }}>
            {paymentMethod === 'card' && (<div className="space-y-4 p-6 bg-slate-50 border border-slate-200"><div className="grid grid-cols-1 gap-4"><FormInput label="Cardholder Name" name="cardholderName" placeholder="John M. Doe" value={formData.cardholderName} onChange={handleInputChange} disabled={isProcessing}/><FormInput label="Card Number" name="cardNumber" placeholder="1234 5678 9012 3456" value={formData.cardNumber} onChange={handleInputChange} disabled={isProcessing}/></div><div className="grid grid-cols-2 gap-4"><FormInput label="Expiry Date" name="expiryDate" placeholder="MM / YY" value={formData.expiryDate} onChange={handleInputChange} disabled={isProcessing}/><FormInput label="CVV" name="cvv" placeholder="123" value={formData.cvv} onChange={handleInputChange} disabled={isProcessing}/></div></div>)}
            {paymentMethod === 'paypal' && (<div className="p-6 bg-slate-50 border border-slate-200 text-center text-slate-700"><p className="font-medium">You will be redirected to PayPal to complete your payment securely.</p></div>)}
            {paymentMethod === 'bank' && (<div className="p-6 bg-slate-50 border border-slate-200 text-center text-slate-700"><p className="font-medium">Please follow the instructions in the confirmation email to complete your bank transfer.</p></div>)}
          </motion.div>
        </AnimatePresence>
      </section>
    </form>
