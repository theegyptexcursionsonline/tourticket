...
const Top10Card = ({ tour, index }: { tour: Tour, index: number }) => {
  ...
  return (
    <a href={`/tours/${tour.slug}`} className="flex items-center gap-6 p-4 bg-white hover:bg-slate-50 transition-colors duration-200 group">
...
const CombiDealCard = ({ tour }: { tour: Tour }) => {
  ...
  return (
    <a href={`/tours/${tour.slug}`} className="w-80 flex-shrink-0 bg-white shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
