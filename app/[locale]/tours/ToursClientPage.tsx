...
type ToursPageCopy = {
  heroTitle: string;
  heroSubtitle: (tourCount: number, destinationCount: number) => string;
  ...
  showingResults: (filtered: number, total: number) => string;
  noToursFoundTitle: string;
  noToursFoundSubtitle: string;
...
const TOURS_PAGE_COPY: Record<'en' | 'ar', ToursPageCopy> = {
  en: {
    ...
    heroSubtitle: (tourCount: number, destinationCount: number) =>
      `Discover ${tourCount} handpicked experiences across ${destinationCount} destinations. Your next adventure starts here!`,
    ...
    showingResults: (filtered: number, total: number) =>
      `Showing ${filtered} of ${total} tours`,
    noToursFoundTitle: 'No tours found',
    noToursFoundSubtitle: 'Try adjusting your filters or search query',
  ...
  ar: {
    ...
    heroSubtitle: (tourCount: number, destinationCount: number) =>
      `اكتشف ${tourCount} تجربة مختارة بعناية عبر ${destinationCount} وجهة. مغامرتك القادمة تبدأ من هنا!`,
    searchPlaceholder: 'ابحث عن الجولات بالاسم أو الوصف...',
    filters: 'الفلاتر',
    clearAll: 'مسح الكل',
    sortBy: 'ترتيب حسب:',
    ...
    showingResults: (filtered: number, total: number) =>
      `عرض ${filtered} من أصل ${total} جولة`,
    noToursFoundTitle: 'لم يتم العثور على جولات',
    noToursFoundSubtitle: 'جرّب تعديل الفلاتر أو عبارة البحث',
      ...
      <div className="container mx-auto max-w-7xl px-4 py-8 sm:py-12">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              ...
              >
                ...
                <option value="price-high">{copy.priceHighToLow}</option>
        ...
        <div className="mb-6">
          <p className="text-slate-600 font-medium">
            {copy.showingResults(filteredTours.length, tours.length)}
          </p>
        </div>
        ...
        {filteredTours.length > 0 ? (
          ...
          >
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2">{copy.noToursFoundTitle}</h3>
            <p className="text-slate-500 mb-6">{copy.noToursFoundSubtitle}</p>
            <button
              onClick={() => {
                setSearchQuery('');
                setSelectedDestination('all');
