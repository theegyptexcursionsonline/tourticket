...
// Serves items an admin set to the "/experience/{slug}" URL type; anything else
// with this slug 301-redirects to its own canonical URL.
...
export default async function ExperiencePrefixPage({ params }: PageProps) {
  const { slug, locale } = await params;
  const decision = await decideForSegment(slug, SEGMENT, locale);
