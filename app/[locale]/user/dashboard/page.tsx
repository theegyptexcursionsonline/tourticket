...
const BookingCard = ({ booking }: { booking: PopulatedBooking & { dateString?: string; time?: string; guests?: number } }) => {
  // Prefer dateString (YYYY-MM-DD) for timezone-safe display, fall back to date
  const dateSource = booking.dateString || booking.date || booking.bookingDate;
  const bookingDate = parseLocalDate(dateSource) || new Date(booking.bookingDate);
        ...
        <div className="p-5 flex flex-col flex-grow min-w-0">
          ...
          <div className="space-y-2 mb-4 flex-grow">
            <div className="flex items-center gap-2 text-slate-600 text-sm">
              ...
              <span className="font-medium">{formatDisplayDate(booking.date || booking.bookingDate)}</span>
            </div>
            
            <div className="flex items-center gap-2 text-slate-600 text-sm">
              <Clock size={14} className="flex-shrink-0" />
              <span>{booking.time || booking.bookingTime}</span>
            </div>

            <div className="flex items-center gap-2 text-slate-600 text-sm">
              <Users size={14} className="flex-shrink-0" />
              <span>{(booking.guests || booking.adults || 0)} guest{(booking.guests || booking.adults || 0) > 1 ? 's' : ''}</span>
            </div>
          </div>
...
const DashboardContent = () => {
  ...
  const { upcomingBookings, pastBookings } = useMemo(() => {
    ...
    return bookings.reduce(
      (acc, booking) => {
        const bdate = new Date(booking.date || booking.bookingDate);
        if (bdate >= now) {
