...
export default async function AISearchAgentShowcasePage() {
  const tours = await getPublishedTours();

  return (
    <>
      <Header2 />
      <main className="min-h-screen bg-gradient-to-b from-violet-50 to-white pt-20">
        {/* Hero Section */}
        <section className="relative overflow-hidden py-20 px-4">
          <div className="absolute inset-0 bg-gradient-to-br from-violet-600/10 to-indigo-500/10" />
          <div className="absolute top-0 right-0 w-96 h-96 bg-violet-400/20 rounded-full blur-3xl" />
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {tours.map((tour: any) => {
                ...
                return (
                  <Link
                    key={tour._id}
                    href={`/showcase/ai-search-agent/${tour.slug}`}
                    className="group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl hover:border-violet-200 transition-all duration-300"
                  >
                    {/* Image */}
                    ...
                    {/* Content */}
                    <div className="p-5">
                      <h3 className="font-bold text-gray-900 mb-2 group-hover:text-violet-600 transition-colors line-clamp-2">
                        {tour.title}
                      </h3>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-violet-600 font-semibold text-sm group-hover:underline">
                          View Details &rarr;
                        </span>
                      </div>
                    </div>
