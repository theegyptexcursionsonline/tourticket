...
interface InlineChatShowcaseClientProps {
  tour: ITour;
  reviews: any[];
  widgetConfig: {
    apiUrl: string;
...
export default function InlineChatShowcaseClient({ tour, reviews, widgetConfig }: InlineChatShowcaseClientProps) {
  ...
  useEffect(() => {
    ...
    script.setAttribute('data-placeholder', 'Ask about this tour...');
  ...
  return (
    <div className="pt-20 min-h-screen bg-gray-50">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
          <div className="max-w-7xl mx-auto">
            <nav className="flex items-center gap-2 text-white/80 text-sm mb-4">
              ...
              {destination && (
                <>
                  <Link href={`/destinations/${destination.slug}`} className="hover:text-white transition-colors">{destination.name}</Link>
                  <span>/</span>
                </>
              )}
              <span className="text-emerald-300">{tour.title}</span>
            </nav>
      ...
      {/* Main: 5-col grid — left 3 = tour, right 2 = inline chat */}
      <section className="max-w-7xl mx-auto px-4 md:px-8 py-12">
        <div className="grid lg:grid-cols-5 gap-8">
                      ...
                      <ul className="grid md:grid-cols-2 gap-4">
                        {tour.highlights.map((highlight, index) => (
                          <li key={index} className="flex items-start gap-3">
                            <span className="flex-shrink-0 w-6 h-6 bg-emerald-100 rounded-full flex items-center justify-center mt-0.5">
                      ...
                      {tour.itinerary.map((item: any, index: number) => (
                        <div key={index} className="flex gap-4">
                          <div className="flex-shrink-0">
                            ...
                            {index < tour.itinerary!.length - 1 && <div className="w-px h-full bg-emerald-200 mx-auto mt-2" />}
                          </div>
                          <div className="flex-1 pb-6">
                            <h4 className="font-semibold text-gray-900 mb-2">{item.time || item.title || `Step ${index + 1}`}</h4>
          ...
          {/* Right: Inline Chat Widget Container */}
