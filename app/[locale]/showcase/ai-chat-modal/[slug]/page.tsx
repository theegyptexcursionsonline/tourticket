...
import ChatModalShowcaseClient from './ChatModalShowcaseClient';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Explore ${result.tour.title}. Click to chat for instant help and recommendations.`,
...
export default async function ChatModalShowcasePage({ params }: PageProps) {
  const { slug } = await params;
  const result = await getTourBySlug(slug);
  if (!result) notFound();
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gradient-to-b from-purple-50 to-white">
        <ChatModalShowcaseClient tour={tour} reviews={reviews} widgetConfig={FOXES_SEARCH_CONFIG} />
      </main>
      <Footer />
    </>
  );
