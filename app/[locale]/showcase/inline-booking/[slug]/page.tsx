...
import InlineBookingClientPage from './InlineBookingClientPage';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${tour.title} — Inline Booking | Egypt Excursions Online`,
    description: `Book ${tour.title} with our inline booking widget. ${tour.description?.slice(0, 150)}...`,
    openGraph: {
      title: tour.title,
...
export default async function InlineBookingShowcaseDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const result = await getTourBySlug(slug);
  if (!result) notFound();
