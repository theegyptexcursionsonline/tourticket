...
export default async function AIVoiceAgentShowcasePage() {
  const tours = await getPublishedTours();

  return (
    <>
      <Header2 />
      <main className="min-h-screen bg-gradient-to-b from-sky-50 to-white pt-20">
        {/* Hero Section */}
        <section className="relative overflow-hidden py-20 px-4">
          <div className="absolute inset-0 bg-gradient-to-br from-sky-600/10 to-violet-500/10" />
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {tours.map((tour: any) => {
                ...
                return (
                  <Link
                    key={tour._id}
                    href={`/showcase/ai-voice-agent/${tour.slug}`}
                    className="group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl hover:border-sky-200 transition-all duration-300"
                  >
                    {/* Image */}
