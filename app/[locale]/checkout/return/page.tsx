...
type CheckoutStatus = 'loading' | 'confirmed' | 'processing' | 'open' | 'expired' | 'error';
...
function HostedCheckoutReturnContent() {
  ...
  useEffect(() => {
    ...
    // Poll quickly for the normal webhook path, then back off so a delayed
    // payment method or provider incident cannot exhaust the public limit.
  ...
  const content = status === 'confirmed'
    ...
    : status === 'expired'
      ? {
          ...
          body: 'This Stripe Checkout session expired before payment. Your inventory hold has been released and you can start again.',
        }
      : status === 'error'
        ? {
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-12 sm:py-20">
      <section className="mx-auto max-w-2xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl">
        <div className="bg-slate-950 px-6 py-7 text-white sm:px-9">
          ...
          <p className="mt-3 text-sm text-slate-300">Egypt Excursions Online</p>
        </div>
...
function HostedCheckoutReturnFallback() {
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
      <div className="text-center" aria-live="polite">
        <Loader2 className="mx-auto h-10 w-10 animate-spin text-red-600" />
        <p className="mt-4 text-sm font-bold text-slate-700">Loading secure payment confirmation…</p>
      </div>
    </main>
  );
}
...
export default function HostedCheckoutReturnPage() {
  return (
    <Suspense fallback={<HostedCheckoutReturnFallback />}>
