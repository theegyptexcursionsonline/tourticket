...
// Serves items an admin set to the "/tour/{slug}" URL type; anything else with
// this slug 301-redirects to its own canonical URL.
const SEGMENT = 'tour';
...
export default async function TourPrefixPage({ params }: PageProps) {
  const { slug, locale } = await params;
  const decision = await decideForSegment(slug, SEGMENT, locale);
