      ...
      <aside className="relative h-full w-full max-w-2xl overflow-y-auto bg-white shadow-2xl">
        <header className={`sticky top-0 z-10 border-b border-slate-200 border-l-4 bg-white px-5 py-5 sm:px-7 ${indicatorTone[event.outcome] || indicatorTone.recorded}`}>
          <div className="flex items-start justify-between gap-4">
            ...
            <button type="button" autoFocus onClick={onClose} className="rounded-xl p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900" aria-label="Close details"><X className="h-5 w-5" /></button>
          </div>
        </header>
...
function AuditPage() {
  ...
  const exportReport = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `eeo-admin-audit-${new Date().toISOString().slice(0, 10)}.csv`;
