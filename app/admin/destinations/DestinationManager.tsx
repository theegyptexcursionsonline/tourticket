...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  const submitData = {
    ...
    imageMetadata: ensureImageMetadata(formData.imageMetadata, [formData.image, ...formData.images].filter(Boolean)),
    ...
    bestDealTourIds: formData.bestDealTourIds,
    topTourIds: formData.topTourIds.filter((tourId) => !formData.bestDealTourIds.includes(tourId)),
  ...
  if (savedDestination?._id) {
    const selectedTourIds = linkedTours || [];
    ...
    Promise.all([
      ...
      ...(editingDestination ? 
        availableTours
          .filter(t => !selectedTourIds.includes(t._id))
          .map(async (tour) => {
...
setTimeout(() => router.refresh(), 0);
  ...
  const toggleCuratedTour = (field: 'bestDealTourIds' | 'topTourIds', tourId: string) => {
    setFormData((prev) => {
      ...
      const next = selected
        ? prev[field].filter((id) => id !== tourId)
        : [...prev[field], tourId].slice(0, limit);
      const otherField = field === 'bestDealTourIds' ? 'topTourIds' : 'bestDealTourIds';
      return {
        ...prev,
        [field]: next,
        [otherField]: selected ? prev[otherField] : prev[otherField].filter((id) => id !== tourId),
      };
    });
  };
