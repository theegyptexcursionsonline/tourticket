...
export default function InternalLinksAdminPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/internal-link-block')
      .then(async (response) => {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const response = await fetch('/api/admin/internal-link-block', {
        method: 'PUT',
        ...
        <div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-slate-600">A reusable, translated discovery block shown before the footer on every public page.</p>
            </div>
          </div>
        </div>
