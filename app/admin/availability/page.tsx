  ...
  useEffect(() => {
    const fetchTours = async () => {
      ...
      try {
        ...
        if (data.success) {
          const tourList = (data.data || []).map((t: { id: string; title: string }) => ({
            _id: t.id,
            title: t.title,
          }));
          setTours(tourList);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Manage tour availability and stop-sales</p>
          </div>
        </div>
      ...
      <div className="mt-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
        ...
        <ul className="text-sm text-slate-600 space-y-1">
          <li>-- <strong>Click</strong> a date to manage option-level stop-sale and slots</li>
          ...
          <li>-- Use the <strong>Stop Sale</strong> button for date-range stop-sale with option-level control</li>
        </ul>
      </div>
...
function StopSaleRangeModal({
  ...
  const submit = async (action: 'block' | 'unblock') => {
    ...
    try {
      ...
      toast.success(action === 'block' ? 'Stop-sale applied' : 'Stop-sale removed');
      onApplied();
    } catch {
...
function SlotEditorModal({
  ...
  // Fetch tour options for option-level stop-sale control
  useEffect(() => {
    ...
    try {
      const res = await fetch('/api/admin/availability', {
        ...
        body: JSON.stringify({
          ...
          stopSaleReason: stopSaleReason || (enabled ? 'Blocked' : ''),
        }),
      });
      const json = await res.json();
      if (!json?.success) {
        ...
        setStopSale(enabled);
        setStopSaleStatus(enabled ? 'full' : 'none');
        if (!enabled) {
          setStoppedOptionIds(new Set());
        }
      }
    } catch {
      toast.error('Failed to update stop-sale');
