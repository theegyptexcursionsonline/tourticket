        ...
        <div className="flex items-center gap-3 w-full md:max-w-lg">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <select
              ...
              className="px-3 py-2 border rounded-md bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-400"
            >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`inline-flex items-center gap-2 px-3 py-2 rounded-md border transition-colors ${
              view === 'table' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white hover:bg-slate-50 border-slate-200'
            }`}
          >
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 px-3 py-2 rounded-md border transition-colors ${
              view === 'cards' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white hover:bg-slate-50 border-slate-200'
            }`}
          >
      ...
      {view === 'table' ? (
        <div className="bg-white border border-slate-100 rounded-lg shadow-sm overflow-x-auto">
          <table className="min-w-full">
            ...
            <tbody>
              {paginated.map((t) => (
                <tr key={t._id} className="border-t last:border-b hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        {/* FIXED: Consistent edit link */}
                        <Link 
                          href={`/admin/tours/edit/${t._id}`} 
                          className="text-sm font-medium text-slate-900 hover:text-red-600 transition-colors block truncate"
                          title={t.title || t.name}
                        >
                          {t.title || t.name}
                        </Link>
                        <div className="text-xs text-slate-500 mt-0.5">
                          {t.duration} {t.isFeatured && <Badge className="ml-2 bg-yellow-100 text-yellow-800">Featured</Badge>}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-700">{t.destination?.name || 'N/A'}</td>
                  <td className="px-4 py-3 text-sm text-slate-700">{t.category?.name || 'N/A'}</td>
              ...
              {paginated.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center text-slate-400">
                    {query ? 'No tours found matching your search.' : 'No tours found.'}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {paginated.map((t) => (
            <div key={t._id} className="bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <Link 
                      href={`/admin/tours/edit/${t._id}`} 
                      className="text-sm font-semibold text-slate-900 hover:text-red-600 transition-colors block truncate"
                      title={t.title || t.name}
                    >
                      {t.title || t.name}
                    </Link>
                    <div className="text-xs text-slate-500 mt-1 truncate">{t.destination?.name || 'N/A'}</div>
                    <div className="text-xs text-slate-500 truncate">{t.category?.name || 'N/A'}</div>
                  </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-6 pt-4 border-t">
          <div className="text-sm text-slate-500">
            Showing {((page - 1) * perPage) + 1} to {Math.min(page * perPage, total)} of {total} tours
          </div>
          <div className="flex items-center gap-2">
            <button 
              onClick={() => setPage((p) => Math.max(1, p - 1))} 
              disabled={page === 1} 
              className="px-3 py-1.5 rounded-md border bg-white hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm"
            >
              Previous
            </button>
