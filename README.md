...
The maintained ten-tool catalog is available at `/[locale]/tools`; every
official tool resolves through `/[locale]/tools/[tool]` (the default English
locale omits the `/en` prefix). A tool page first requests Authority
configuration on the server with `X-Publisher-Token` and the fixed flagship
host. It renders the Authority embed only when publisher attribution, tenant
branding, source freshness and the configured embed origin all verify. Missing
or invalid configuration fails closed with an unavailable state; no local price
or policy fallback is used. The publisher token is never written to a URL,
browser script or rendered HTML.
