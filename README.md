...
- `OPENAI_API_KEY`
- Content Engine receiver: `CONTENT_ENGINE_ALLOWED_TENANTS=default`, one or more
...
#### Content Engine receiver grants
...
The registry names secret environment variables; it never contains their
values. IDs, secret variable names, resolved secrets and targets within a grant
...
second `CONTENT_ENGINE_API_KEY_*` secret and grant, switch the caller, then
remove the old grant and secret after adoption is verified. A bare legacy key
can still authenticate lookup routes but cannot authorize an unbound mutation;
