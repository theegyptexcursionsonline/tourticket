export const INTERNAL_LINK_LOCALES = ['en', 'ar', 'de', 'fr', 'es', 'ru'] as const;
...
function localizedText(value: unknown, maxLength: number): LocalizedText {
  ...
  return Object.fromEntries(
    INTERNAL_LINK_LOCALES
      .map((locale) => [locale, text(source[locale], maxLength)] as const)
...
function uniqueId(candidate: string, used: Set<string>, fallbackIndex: number): string {
...
export function sanitizeInternalLinkBlock(value: unknown): InternalLinkBlockValue {
  const source = value && typeof value === 'object' ? value as Record<string, unknown> : {};
  const rawGroups = Array.isArray(source.groups) ? source.groups.slice(0, 8) : [];
  const usedGroupIds = new Set<string>();

  const groups = rawGroups.map((entry, groupIndex) => {
    const group = entry && typeof entry === 'object' ? entry as Record<string, unknown> : {};
    const id = uniqueId(stableId(group.id, `group-${groupIndex + 1}`), usedGroupIds, groupIndex);
    usedGroupIds.add(id);
    ...
    const links = (Array.isArray(group.links) ? group.links : [])
      ...
      .map((rawLink, linkIndex) => {
        ...
        const linkId = uniqueId(stableId(link.id, `${id}-link-${linkIndex + 1}`), usedLinkIds, linkIndex);
...
function translated(value: LocalizedText, locale: string): string {
  const normalized = INTERNAL_LINK_LOCALES.includes(locale as InternalLinkLocale)
    ...
    groups: value.groups
      ...
      .map((group) => ({
        ...
        links: group.links
          ...
          .map((link) => ({ id: link.id, label: translated(link.label, locale), href: link.href }))
