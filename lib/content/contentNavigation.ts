...
export function sanitizeContentNavigation(input: unknown): ContentNavigationValue {
  if (!input || typeof input !== 'object') return {};
  const value = input as Record<string, unknown>;
  const hasBreadcrumbLabel = Object.prototype.hasOwnProperty.call(value, 'breadcrumbLabel');
  const hasParentPage = Object.prototype.hasOwnProperty.call(value, 'parentPage');
  const breadcrumbLabel = typeof value.breadcrumbLabel === 'string'
  ...
  if (!rawParent || typeof rawParent !== 'object') {
    return { ...(hasBreadcrumbLabel ? { breadcrumbLabel: breadcrumbLabel || '' } : {}), parentPage: null };
  }

  const parent = rawParent as Record<string, unknown>;
  const slug = typeof parent.slug === 'string' ? parent.slug.trim().toLowerCase() : '';
  const label = typeof parent.label === 'string' ? parent.label.trim().slice(0, 120) : '';
  const kind = typeof parent.kind === 'string' ? parent.kind : '';
  const id = typeof parent.id === 'string' ? parent.id.trim() : '';

  if (!SAFE_SLUG.test(slug) || !label || !(PARENT_CONTENT_KINDS as readonly string[]).includes(kind)) {
    return { ...(hasBreadcrumbLabel ? { breadcrumbLabel: breadcrumbLabel || '' } : {}), parentPage: null };
  }
...
export function nestedContentPath(slug: string, parentPage?: ParentPageValue | null): string | null {
