...
const TourTranslationSchema = new Schema<ITourTranslation>(
  {
    ...
    whatsIncluded: [{ type: String, trim: true, maxlength: 300 }],
    whatsNotIncluded: [{ type: String, trim: true, maxlength: 300 }],
...
TourSchema.statics.generateUniqueSlug = async function(baseSlug: string, excludeId?: string) {
  ...
  while (!(await (this as any).isSlugAvailable(slug, excludeId))) {
    slug = `${baseSlug}-${counter}`;
    counter++;
