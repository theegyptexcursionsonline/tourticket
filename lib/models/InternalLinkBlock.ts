...
import type { InternalLinkBlockValue, LocalizedText } from '@/lib/navigation/internalLinks';
...
const InternalLinkBlockSchema = new Schema<IInternalLinkBlock>({
  ...
  enabled: { type: Boolean, default: true },
  heading: { type: LocalizedTextSchema, default: () => ({ en: 'Explore Egypt' }) },
  groups: { type: [GroupSchema], default: [] },
}, { timestamps: true });

const InternalLinkBlock: Model<IInternalLinkBlock> =
  (mongoose.models.InternalLinkBlock as Model<IInternalLinkBlock>)
  || mongoose.model<IInternalLinkBlock>('InternalLinkBlock', InternalLinkBlockSchema);
