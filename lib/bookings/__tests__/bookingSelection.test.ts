...
describe('booking selection isolation', () => {
  const standard = {
    id: 'standard',
    timeSlots: bindTimeSlotsToOption('standard', [{ id: '10:00', price: 80 }]),
  };
  const privateOption = {
    id: 'private',
    timeSlots: bindTimeSlotsToOption('private', [{ id: '10:00', price: 170 }]),
  };
  ...
  it('resolves checkout pricing from the owning option', () => {
    expect(findSelectedBookingOption([standard, privateOption], privateOption.timeSlots[0]))
      .toBe(privateOption);
  });

  it('turns an unselected add-on on with one stored unit', () => {
    expect(nextAddOnSelectionQuantity(0)).toBe(1);
    expect(nextAddOnSelectionQuantity(1)).toBe(0);
  });
});
