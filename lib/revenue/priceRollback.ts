...
export async function rollbackPriceExecution(
  ...
  const original = await RevenuePriceExecution.findOne({ executionId, tenantId: 'default' }).lean<IRevenuePriceExecution | null>();
  ...
  assertRevenuePilotTourAllowed(String(original.target.tourId));
