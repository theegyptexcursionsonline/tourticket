...
export type PriceWrite = {
  ...
  policyHash: string; policySnapshot: { floor: number; ceiling: number; maxChangePercent: number; minConfidence: number; cooldownHours: number; mode: 'manual' | 'assist' | 'autopilot' }; sourceVersion: string; confidence: number; actor: string; mode: 'manual' | 'assist' | 'autopilot';
};
...
export function validatePriceWrite(value: unknown): PriceWrite {
  ...
  if (!policy || !Number.isFinite(policy.floor) || !Number.isFinite(policy.ceiling) || policy.floor < 0 || policy.floor >= policy.ceiling || !Number.isFinite(policy.maxChangePercent) || policy.maxChangePercent < 1 || policy.maxChangePercent > 5 || !Number.isFinite(policy.minConfidence) || policy.minConfidence < 85 || policy.minConfidence > 100 || !Number.isInteger(policy.cooldownHours) || policy.cooldownHours < 24 || policy.cooldownHours > 168 || !['manual', 'assist', 'autopilot'].includes(policy.mode)) throw new Error('Invalid policy snapshot');
