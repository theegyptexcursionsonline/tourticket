...
export type RevenueCommissioningTarget = { tourId: string; optionKey: string; date: string; time: string };

export function parseRevenueCommissioningTarget(raw?: string) {
  const value = raw?.trim();
  if (!value) return null;
  const [tourId, optionKey, date, time, ...extra] = value.split('|').map((part) => part.trim());
  if (extra.length || !TOUR_OBJECT_ID.test(tourId || '') || !OPTION_KEY.test(optionKey || '') || !/^\d{4}-\d{2}-\d{2}$/.test(date || '') || !/^(?:[01]\d|2[0-3]):[0-5]\d$/.test(time || '')) {
    throw new RevenuePricingWriteError(503, 'COMMISSIONING_TARGET_INVALID', 'REVENUEPILOT_COMMISSIONING_TARGET must be one exact tourId|optionKey|YYYY-MM-DD|HH:mm target.');
  }
  return { tourId: tourId!.toLowerCase(), optionKey: optionKey!, date: date!, time: time! };
}
...
export function revenueCommissioningArmState(env: NodeJS.ProcessEnv = process.env, now = new Date()) {
  ...
  const blockReason = env.REVENUEPILOT_COMMISSIONING_ENABLED !== 'true'
    ? 'globally_disabled'
    : env.REVENUEPILOT_COMMISSIONING_CONFIRMATION !== 'ENABLE_EEO_ONE_TIME_COMMISSIONING'
...
export function assertRevenuePilotCommissioningAllowed(input: PriceWrite, env: NodeJS.ProcessEnv = process.env, now = new Date()) {
  ...
  if (!targetMatches(arm.target, input.target)) throw new RevenuePricingWriteError(403, 'COMMISSIONING_TARGET_FORBIDDEN', 'This exact tour, option, date and time is not commissioned.');
}
