...
jest.mock('@/lib/revenue/pricingSummary', () => {
  return {
    ...
    pricingProjectionStatus: (tour: {
      ...
      pricingSearchProjection?: { status?: string; summaryVersion?: number; authoritativeVersion?: number };
    } | null, authoritativeVersion?: number) => {
      const summaryVersion = Number(tour?.pricingSummary?.version ?? -1);
      const projectionVersion = Number(tour?.pricingSearchProjection?.summaryVersion ?? -1);
      const versionMatches = summaryVersion >= 0
        && summaryVersion === projectionVersion
        && (authoritativeVersion === undefined || Number(tour?.pricingSearchProjection?.authoritativeVersion ?? -1) === authoritativeVersion);
      const verified = tour?.pricingSearchProjection?.status === 'verified' && versionMatches;
      return {
        state: verified ? 'verified' : tour?.pricingSearchProjection?.status === 'failed' ? 'failed' : 'pending',
        verified,
        versionMatches,
      };
    },
...
describe('RevenuePilot projection replay routes', () => {
  ...
  beforeEach(() => {
    ...
    mockAuthenticateRevenueRequest.mockResolvedValue({ response: null });
  ...
  it('repairs the listing/search projection when an apply is replayed', async () => {
    ...
    expect(mockReconcileTourPricingProjection).toHaveBeenCalledWith(priceInput.target.tourId, 'USD', 3);
  ...
  it('repairs the listing/search projection when a rollback is replayed', async () => {
    ...
    expect(mockReconcileTourPricingProjection).toHaveBeenCalledWith(priceInput.target.tourId, 'USD', 4);
    expect(body.channelPropagation.eeo_direct).toBe('verified');
