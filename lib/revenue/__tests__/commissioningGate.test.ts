...
const target = { tourId: '507f1f77bcf86cd799439011', optionKey: 'sunrise_private', date: '2026-08-10', time: '06:30' };
...
describe('RevenuePilot commissioning gate', () => {
  it('parses and requires one exact target', () => {
    expect(parseRevenueCommissioningTarget(`${target.tourId}|${target.optionKey}|${target.date}|${target.time}`)).toEqual(target);
    expect(() => parseRevenueCommissioningTarget(`*|standard|${target.date}|${target.time}`)).toThrow(/exact/);
  });
  ...
  it('requires a confirmed, future window no longer than 24 hours', () => {
    const env = {
      ...
      REVENUEPILOT_COMMISSIONING_TARGET: `${target.tourId}|${target.optionKey}|${target.date}|${target.time}`,
