...
function captureAuditInput(request: NextRequest): Promise<CapturedAuditInput> {
  ...
  return clone.json()
    .then((body: unknown) => {
      ...
      // `code` is normally a useful safe enum/status field, but on the 2FA
      // endpoint it is the administrator's one-time authenticator credential.
...
async function targetTenantIds(
  ...
  if (auth.role === 'super_admin' || auth.role === 'system') {
