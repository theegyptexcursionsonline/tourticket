...
} from '@/lib/auth/adminPortalScope';
...
export function revokePortalScope(
  ...
  // A normal network member needs at least one assigned brand. A super-admin
  // is intentionally global and therefore keeps network access even without
  // tenantIds. Removing only some brands must not drop either kind of member
  // from the network while access remains.
