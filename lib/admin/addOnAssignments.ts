...
type AddOnWithAssignments = Record<string, unknown> & {
...
/**
 * Converts editor-only option identities to the durable pricing keys minted by
 * the server. Unknown/stale targets are dropped fail-closed. An empty list
 * retains the backwards-compatible "all options" meaning.
 */
export function finalizeAddOnAssignments(
  ...
  const durableKeys = new Set(bookingOptions.map((option) => text(option.pricingKey)).filter(Boolean) as string[]);
  const aliases = new Map<string, string>();
  for (const option of bookingOptions) {
    const durable = text(option.pricingKey);
    if (!durable) continue;
