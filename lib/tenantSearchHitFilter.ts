...
const getTranslationBucket = (hit: TenantSearchHit, locale: string) => {
  ...
  return (
    asRecord(translations[normalizedLocale]) ||
    asRecord(translations[normalizedLocale.split('-')[0]])
  );
};
...
const localizeTourSearchHit = <T extends TenantSearchHit>(hit: T, locale: string): T => {
  ...
  return Object.keys(localizedFields).length > 0
    ? ({ ...hit, ...localizedFields } as T)
    : hit;
};
...
/**
 * Scope tour search hits to a tenant, collapse translated duplicate records by
 * storefront route, and return fields localized for the active language.
 * English storefronts also reject German-only orphan hits instead of leaking
 * them into search when no English source record exists.
 */
export function filterTourSearchHitsByTenant<T extends TenantSearchHit>(
  ...
  const localized = Array.from(bestByRoute.values()).map((hit) =>
    localizeTourSearchHit(hit, locale)
  );
