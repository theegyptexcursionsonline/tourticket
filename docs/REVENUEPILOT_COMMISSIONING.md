...
TourTicket supports a separately armed, exact-target commissioning request so the EEO owner can prove one RevenuePilot write/read-back/restore path without enabling normal pricing execution.
...
REVENUEPILOT_COMMISSIONING_CONFIRMATION=ENABLE_EEO_ONE_TIME_COMMISSIONING
REVENUEPILOT_COMMISSIONING_TARGET=<tourId>|<optionKey>|<YYYY-MM-DD>|<HH:mm>
...
This gate does not create bookings or payments, does not enable ordinary RevenuePilot pricing, and does not apply to sibling EEO repositories. Follow the release-owner procedure in RevenuePilot `docs/EEO_COMMISSIONING_RUNBOOK.md`; immediately disarm the four variables after the test.
