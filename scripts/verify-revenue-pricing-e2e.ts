...
function signedRequest(method: string, path: string, body = '', keyId = 'primary', nonce = randomUUID(), extraHeaders: Record<string, string> = {}) {
  ...
  const signature = createHmac('sha256', secret).update([timestamp, nonce, method, path, bodyHash].join('\n')).digest('hex');
...
async function run() {
  ...
  const publicQuote = await json(await quote(new NextRequest(`http://localhost${quotePath}`), { params: Promise.resolve({ tourId: String(tour._id) }) }));
  ...
  await StopSale.create({ tourId: tour._id, startDate: stopDate, endDate: stopDate, optionIds: ['option-0'], reason: 'Local E2E option stop sale' });
  ...
  const restoredQuote = await json(await quote(new NextRequest(`http://localhost${quotePath}`), { params: Promise.resolve({ tourId: String(tour._id) }) }));
