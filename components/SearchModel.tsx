...
const TourResultCard: FC<{ tour: Tour }> = ({ tour }) => (
    <a href={`/tour/${tour.slug}`} className="group block bg-white rounded-lg shadow-md overflow-hidden transition-shadow hover:shadow-xl">
        ...
        <div className="p-4">
            <h4 className="font-bold text-gray-900 group-hover:text-red-500 truncate text-lg mb-1">{tour.title || 'Untitled Tour'}</h4>
            <p className="text-sm text-gray-500 mb-2">{tour.destination?.name || 'Unknown Destination'}</p>
            {tour.rating ? (
                <div className="flex items-center">
                    <Star className="w-5 h-5 text-yellow-400 mr-1" />
                    <span className="text-gray-800 font-bold">{tour.rating.toFixed(1)}</span>
                    {tour.reviews && <span className="text-gray-500 text-sm ml-2">({tour.reviews.length} reviews)</span>}
                </div>
            ) : (
                    ...
                    <div className="mb-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {searchResults.map(tour => (
                                <TourResultCard key={tour._id} tour={tour} />
                            ))}
                        </div>
                    </div>
                )}
