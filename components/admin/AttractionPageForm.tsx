...
export default function AttractionPageForm({ pageId, initialPageType = 'attraction' }: AttractionPageFormProps) {
  ...
  const [selectedTours, setSelectedTours] = useState<ContentListingOption[]>([]);
      ...
      if (data.success) {
        ...
        if (linkedTourIds.length > 0) {
          fetch(`/api/admin/pages/options?kind=tours&ids=${linkedTourIds.join(',')}`)
            ...
            .then((json) => {
              if (json.success) {
                const byId = new Map((json.data as ContentListingOption[]).map((option) => [option.id, option]));
                setSelectedTours(linkedTourIds.map((tourId) => byId.get(tourId)).filter(Boolean) as ContentListingOption[]);
              }
            })
            .catch(() => {});
        }
        ...
        if (linkedRefIds.length > 0) {
          fetch(`/api/admin/pages/options?kind=pages&ids=${linkedRefIds.join(',')}`)
            ...
            .then((json) => {
              if (json.success) {
                const byId = new Map((json.data as ContentListingOption[]).map((option) => [option.id, option]));
                setSelectedPages(linkedRefIds.map((refId) => byId.get(refId)).filter(Boolean) as ContentListingOption[]);
              }
            })
            .catch(() => {});
        }
