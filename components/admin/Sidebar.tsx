    ...
    <>
      {isNavigating && (
        <>
          <div className="fixed inset-x-0 top-0 z-[80] h-1 bg-slate-200">
            <div className="h-full w-1/2 animate-pulse rounded-r-full bg-gradient-to-r from-blue-500 via-purple-500 to-blue-500" />
          </div>
          <div className="fixed bottom-6 right-6 z-[80] flex items-center gap-3 rounded-2xl border border-blue-100 bg-white/95 px-4 py-3 text-sm font-semibold text-slate-700 shadow-2xl backdrop-blur">
      ...
      {isMobile && (
        <button
          ...
          className="fixed top-3 left-3 z-[70] flex items-center justify-center h-11 w-11 rounded-xl bg-white shadow-lg border border-slate-200 text-slate-600 hover:text-slate-900 transition-all duration-200 lg:hidden"
        >
      ...
      {/* Sidebar */}
      <aside
        className={`bg-white border-r border-slate-200/60 backdrop-blur-sm flex flex-col transition-all duration-300 ease-out shadow-lg overflow-hidden flex-shrink-0 h-full ${sidebarWidth} ${mobileClass}`}
