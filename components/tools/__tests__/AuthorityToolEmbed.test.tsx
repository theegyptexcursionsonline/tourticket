...
describe('AuthorityToolEmbed', () => {
  ...
  it('requires its signed resize handshake and accepts bounded messages only from its own frame', async () => {
  ...
  it('shows a usable error state when the iframe never completes its readiness handshake', async () => {
    jest.useFakeTimers();
    render(<AuthorityToolEmbed src={SRC} tool="visa-checker" title="Egypt Visa & Entry Checker" />);
    await act(async () => {
