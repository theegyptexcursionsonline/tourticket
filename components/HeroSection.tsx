...
interface HeroCopy {
  toursExperiences: string;
  handPicked: string;
  destinations: string;
  explorePlaces: string;
  categories: string;
  browseInterest: string;
  found: string;
  featured: string;
  untitledTour: string;
  ...
  aiTravelAssistant: string;
  poweredByAi: string;
  searchResultsTitle: string;
  searchResultsSubtitle: string;
  discoverEgyptTitle: string;
  discoverEgyptSubtitle: string;
  newChat: string;
  welcomeTitle: string;
...
const HERO_COPY: Record<'en' | 'ar', HeroCopy> = {
  en: {
    toursExperiences: 'Tours & Experiences',
    handPicked: 'Hand-picked for you',
    destinations: 'Destinations',
    explorePlaces: 'Explore new places',
    ...
    searchResultsSubtitle: 'Best matches for you',
    discoverEgyptTitle: 'Discover Egypt',
    discoverEgyptSubtitle: 'Trending searches',
    newChat: 'New Chat',
    welcomeTitle: "Hi! I'm your AI Egypt Travel Assistant",
    welcomeSubtitle: "Ask me anything - I'll help you find tours, trips, prices, destinations and more.",
    loadingTitle: 'AI is thinking...',
    loadingSubtitle: 'Finding the best options for you',
    trendingTitle: 'Trending Egypt Tours',
    trendingSubtitle: 'Most searched this week',
    askAiCta: 'Ask AI Travel Assistant',
    askAiSubtext: 'Get personalized tour recommendations instantly',
    starterPrompts: [
  ...
  ar: {
    ...
    explorePlaces: 'استكشف أماكن جديدة',
    categories: 'الفئات',
    ...
    openAiAssistantAria: 'فتح مساعد الذكاء الاصطناعي',
    aiTravelAssistant: 'مساعد السفر بالذكاء الاصطناعي',
    poweredByAi: 'مدعوم بالذكاء الاصطناعي',
    searchResultsTitle: 'نتائج البحث',
    searchResultsSubtitle: 'أفضل النتائج لك',
    discoverEgyptTitle: 'اكتشف مصر',
    discoverEgyptSubtitle: 'عمليات البحث الرائجة',
    newChat: 'محادثة جديدة',
    welcomeTitle: 'مرحبًا! أنا مساعد السفر الذكي الخاص بمصر',
    welcomeSubtitle: 'اسألني أي شيء وسأساعدك في العثور على جولات ورحلات وأسعار ووجهات وأكثر.',
    loadingTitle: 'الذكاء الاصطناعي يفكر...',
    loadingSubtitle: 'يبحث عن أفضل الخيارات لك',
    trendingTitle: 'الجولات الرائجة في مصر',
    trendingSubtitle: 'الأكثر بحثًا هذا الأسبوع',
    askAiCta: 'اسأل مساعد السفر الذكي',
    askAiSubtext: 'احصل على توصيات جولات مخصصة فورًا',
    starterPrompts: [
      'رشّح لي أفضل رحلة نيلية بأقل من 300 دولار',
      'خطط لي برنامج 7 أيام في مصر',
      'ما أفضل الجولات بالقرب من القاهرة؟',
    ],
  },
};
...
const useHeroCopy = () => {
  ...
  return locale.startsWith('ar') ? HERO_COPY.ar : HERO_COPY.en;
};
...
const useSlidingText = (
  ...
  return safeTexts[currentIndex] || safeTexts[0] || fallbackText;
};
            ...
            >
              <div className="relative">
                <input
                  ...
                  ref={inputRef}
                  placeholder={chatMode ? aiPlaceholder : suggestion}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {chatMode ? (
                      ...
                      >
                        ...
                        <div>
                          <span className="text-sm font-bold text-gray-900 tracking-tight block drop-shadow-sm">
                            {query ? copy.searchResultsTitle : copy.discoverEgyptTitle}
                          </span>
                          <span className="text-[10px] text-gray-600 font-medium drop-shadow-sm">
                            {query ? copy.searchResultsSubtitle : copy.discoverEgyptSubtitle}
                          </span>
                        </div>
                      </motion.div>
                    )}
                  ...
                  >
                    ...
                    {isGenerating && (
                      ...
                      >
                        ...
                        <div>
                          <span className="text-sm font-semibold text-gray-900">{copy.loadingTitle}</span>
                          <p className="text-[10px] text-gray-500">{copy.loadingSubtitle}</p>
                        </div>
                      </motion.div>
                    )}
                  </motion.div>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="text-sm font-bold relative z-10">{copy.askAiCta}</span>
                        <motion.div
                          animate={{ x: [0, 4, 0] }}
...
export default function HeroSection({ initialSettings }: HeroSectionProps = {}) {
  ...
  // Use CMS text for English, localized strings for non-English locales.
  const heroMainText = isEnglishLocale
    ? (settings.title.main || tHero('defaultTitle'))
    : tHero('defaultTitle');
  const heroHighlightText = isEnglishLocale
    ? (settings.title.highlight || tHero('defaultHighlight'))
    : tHero('defaultHighlight');
  const heroSubtitleText = isEnglishLocale
    ? (settings.metaDescription || tHero('subtitle'))
    : tHero('subtitle');
  const travelersText = isEnglishLocale
    ? (settings.trustIndicators.travelers || tHero('travelers'))
    : tHero('travelers');
  const ratingStarsText = isEnglishLocale
    ? (settings.trustIndicators.ratingText || tHero('ratingStars'))
    : tHero('ratingStars');
  const ratingScoreText = isEnglishLocale
    ? (settings.trustIndicators.rating || tHero('ratingScore'))
    : tHero('ratingScore');
  const effectiveSearchSuggestions =
    isEnglishLocale && settings.searchSuggestions.length > 0
      ? settings.searchSuggestions
      : [tHero('searchPlaceholder')];

  const currentSuggestion = useSlidingText(
    effectiveSearchSuggestions,
    3000,
    tHero('searchPlaceholder')
  );
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center h-full text-center md:items-start md:text-left" style={{ overflow: 'visible' }}>
          <div className="max-w-xl" style={{ overflow: 'visible', position: 'relative' }}>
            ...
            <HeroSearchBar
              ...
              aiPlaceholder={tHero('aiPlaceholder')}
            />
