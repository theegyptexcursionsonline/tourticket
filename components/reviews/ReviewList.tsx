...
const ReviewList: React.FC<ReviewListProps> = ({ reviews, onReviewUpdated, onReviewDeleted }) => {
  ...
  const handleUpdate = async (reviewId: string) => {
    if (!token || !editComment.trim() || editRating === 0) {
      toast.error(t('toasts.provideRatingAndComment'));
      return;
    }
  ...
  const isReviewOwner = (review: Review) => {
    ...
    return (
      user &&
      reviewUser &&
      (reviewUser._id === (user.id || user._id) || reviewUser.id === (user.id || user._id))
    );
  };
                        ...
                        {editingReview === review._id ? (
                          <>
                            <button
                              ...
                              aria-label={t('actions.saveReview')}
                            >
                          ...
                          <>
                            <button
                              ...
                              title={t('actions.editReview')}
                              aria-label={t('actions.editReview')}
                            >
                            ...
                            <button
                              ...
                              title={t('actions.deleteReview')}
                              aria-label={t('actions.deleteReview')}
                            >
                ...
                {editingReview === review._id ? (
                  <div className="mt-4">
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      <p className="text-xs text-gray-500">{t('characters', { count: editComment.length, max: 1000 })}</p>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={cancelEdit}
                          disabled={isUpdating}
                  ...
                  <>
                    {review.comment ? (
                      ...
                      <p className="mt-3 text-gray-500 italic text-sm">{t('ratedOnly')}</p>
                    )}
                  </>
                )}
              </div>
